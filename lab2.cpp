...
int elementFinder(int arr[], int n, int value) {
    for (int i = 0; i < n; ++i) {
        if (value == arr[i]) {
            return 1;
        }
...
void sortedMassive(int arr[], const int n){
    ...
    result = end - start;
    cout << "\nmin: " << mn << ", max: " << mx;
    cout << "\nПоиск max и min значений отсортированного массива занял " << result.count() << " наносекунд\n";
    //4 TASK
    avg = round((mx + mn) / 2.0);
    ...
    while (index1 != -1 && index2 != -1) {
        ...
        cout <<"\nМассив с индексами, которые поменяли местами :";
        for (int i = 1; i < n; ++i) {
            cout << " " << arr[i];
        }
        cout << "\nЗамена прошла за " << (end - start).count() << " наносекунд\n";
        cin >> index1 >> index2;
    }
}
 //---------------------------------------------------------------------------
